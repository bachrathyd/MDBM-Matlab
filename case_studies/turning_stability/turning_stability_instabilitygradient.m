...
par.zeta=0.05;
par.procdamp=0.005;
par.constraintonly=false;

%% number of iteration
Niteration=4;
%% function for which the roots are detected
bound_fuction_name='chareq_turning_stability';
...
ghandle=plot_mdbm(mdbm_sol,[],[1,2,3],[],[],ax);
set(ghandle, 'LineWidth',2)
view(0,0)
xlabel('Dimensionless spindle speed')
zlabel('chatter frequency')
...
mdbm_sol=refine(mdbm_sol,[3,3]); % refinement along the spindle speed axis only (however, it is refined twice!!!, because "3" is repeated)
...
par.constraintonly=true; %not the constrain is independent from the omc parameter, thus, only a two parameter computation should be evaluated to have a faster solution
...
legend({'Stability limits (bifurcation curves)','applied constraints','instability gradient'}, 'Location', 'northoutside')
