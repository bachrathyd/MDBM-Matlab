...
% Code damonstrates the effect of the Bezier interpolation of the results
% Note, for the default plotting useing interpolation by default
% (if interpolation order > 0)
ax=[];
...
bound_fuction_name=@foo;%see at the end of the code
...
for iorder=0:2
    ...
    mdbm_sol_interp=interpplot(mdbm_sol,4);
...
%%
mdbm_sol=mdbm(ax,bound_fuction_name,2,mdbmoptions);% no refinement, only initialization!!!
mdbm_sol_no_interp=interpplot(mdbm_sol,2);
for SN=2:12%[3,6,9,12]%10%3:25

    figure(8),clf
    set(gcf,'Position', [100,100,900,600])
    mdbm_sol_interp2=interpplot(mdbm_sol,SN);% this create a bezier interpoltaion for each triangle

    ax2loc2(1)=subplot(1,2,1);
    plot_mdbm(mdbm_sol_no_interp);
