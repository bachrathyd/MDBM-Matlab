...
Ncodim=size(feval(functionname,axtest1,varargin{3:end}),1);
%------------
...
% for kcomb=size(combinations,2):-1:1
  ...
  % ourh=plot_mdbm(mdbm_sols{kcomb},[],[],0,[],[],0);
   ...
   function Hfunctionoutputvalues=wrapper(axvariax,varargin)
      ...
      if ~any(localextraequations==0)
         ...
         for kparam_extraeq=localextraequations'%adding the extra equations
            ...
            HfunctionoutputvaluesDELTA=cat(1,HfunctionoutputvaluesDELTA,(feval(functionname,axloc,varargin{:})-Hfunctionoutputvalues)/epsilon_parturbation);%the approximated derivative
         ...
         for kallpoints=1:size(HfunctionoutputvaluesDELTA,2)
            A=reshape(HfunctionoutputvaluesDELTA(:,kallpoints),size(Hfunctionoutputvalues,1),[]);
         ...
         Hfunctionoutputvalues=cat(1,Hfunctionoutputvalues,added_fun_value);
      end
   end
